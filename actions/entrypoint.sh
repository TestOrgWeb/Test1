...
for i in $backportingBranches; do
    ...
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{ 
        \"body\" : \"Backporting attempted at ${COMMENTTIME} failed for branch $i!! Manual intervention required to resolve the conflicts. Workflow URL - $workflowUrl New PR URL- $pull_url\"
        }"`
    ...
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{ 
        \"body\" : \"Backporting attempted at ${COMMENTTIME} successful for branch $i Workflow URL- $workflowUrl New PR URL- $new_pr\" 
        }"`
